...
sub cmd_greet {
    ...
    given (uc $argv[0]) {
        when ('ADD') {
            ...
            my $sNickname = $argv[1]; shift @argv; shift @argv;
            $sNickname = lc $sNickname;
            my $sGreetMessage = join(' ', @argv);

            # Make sure it doesn't already exist.
            if ($Auto::DB->selectrow_array('SELECT * FROM greets WHERE nick = "'.$sNickname.'"')) {
                notice($src->{svr}, $src->{nick}, "A greet for \002$sNickname\002 already exists.");
                return;
            }
            ...
            $dbq->execute($sNickname, $sGreetMessage) or notice($src->{svr}, $src->{nick}, trans('An error occurred').q{.}) and return;

            API::Log::slog('cmd_greet(): Creating new greet.');
            # Done.
            notice($src->{svr}, $src->{nick}, "Successfully added greet for \002$sNickname\002.");
        }
        when ('DEL') {
            # GREET DEL
            ...
            my $sNickname = lc $argv[1];

            # Check if there is a greet for this user.
            if (!$Auto::DB->selectrow_array('SELECT * FROM greets WHERE nick = "'.$sNickname.'"')) {
                notice($src->{svr}, $src->{nick}, "There is no greet for \002$sNickname\002.");
                return;
            }

            # Delete it.
            $Auto::DB->do('DELETE FROM greets WHERE nick = "'.$sNickname.'"') or notice($src->{svr}, $src->{nick}, trans('An error occurred').q{.}) and return;
            notice($src->{svr}, $src->{nick}, "Greet for \002$sNickname\002 successfully deleted.");
        }
...
sub hook_rcjoin {
    my (($src, $chan)) = @_;
    my $sNickname = lc $src->{nick};

    # Check if there's a greet for this user.
    if ($Auto::DB->selectrow_array('SELECT * FROM greets WHERE nick = "'.$sNickname.'"')) {
        # Get data.
        my @aData = $Auto::DB->selectrow_array('SELECT * FROM greets WHERE nick = "'.$sNickname.'"');
        
        # Send the greet.
        privmsg($src->{svr}, $chan, "[\002".$src->{nick}."\002] ".$aData[1]);
    }
...
API::Std::mod_init('Greet', 'RedStons Development Group', '1.00', '3.0.0a11');
# build: perl=5.010000
